...
// 🔧 ATIVAR MODO DE DEPURAÇÃO DO APP CHECK PARA DESENVOLVIMENTO LOCAL
// Isso deve ser feito ANTES da inicialização do Firebase
if (
  window.location.hostname === "localhost" ||
  window.location.hostname === "127.0.0.1"
) {
  // Ativa o modo de depuração para desenvolvimento local
  self.FIREBASE_APPCHECK_DEBUG_TOKEN = true;
  console.log("🔧 Modo de depuração do App Check ativado para ambiente local");
  console.log(
    "📝 Verifique o console para obter o token de depuração e registre no Firebase Console",
  );
}
  ...
  if (typeof firebase !== "undefined" && firebase.app) {
    ...
    if (!firebase.apps || !firebase.apps.length) {
      ...
      try {
        ...
        console.warn(
          "⚠️ App Check já estava ativado ou erro:",
    ...
    console.log(
      "Verifique se os scripts do Firebase estão sendo carregados antes deste arquivo",
    );
  ...
  return new Promise((resolve) => {
    if (window.isFirebaseReady()) {
      ...
      setTimeout(() => {
        if (!window.isFirebaseReady()) {
          console.warn(
            "⚠️ Timeout aguardando Firebase, continuando mesmo assim...",
          );
          resolve();
        }
        document.removeEventListener("firebase-ready", handler);
